...
pub fn handle_approval_enter_line(line: &str, call_id: &str) -> Result<Option<bool>, &'static str> {
    let t = line.trim();
    if t.is_empty() {
        return Err("empty");
    }
...
mod tests {
    ...
    #[test]
    fn approval_enter_parses_yes() {
        assert_eq!(handle_approval_enter_line("yes", "c1").unwrap(), Some(true));
    }
}
