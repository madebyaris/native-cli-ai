...
use crate::tui::input::{
    ApprovalAnswer, handle_api_key_modal_key, handle_branch_picker_key, handle_command_palette_key,
    handle_connect_modal_key, handle_question_modal_key, parse_tui_question_answer,
    render_branch_picker, render_command_palette,
};
use crate::tui::layout::{
    COMMAND_PALETTE_MAX_ROWS, COMMAND_PALETTE_WIDTH, centered_rect, layout_chunks,
    layout_with_sidebar, rect_contains, sidebar_fit,
};
use crate::tui::state::{
...
/// After the first printable key in the TUI, warm TLS + HTTP to the default provider
/// so the first real chat request pays less connection setup cost.
fn schedule_http_prewarm_on_first_printable_key(
    http_prewarm: &Option<(tokio::runtime::Handle, NcaConfig)>,
    done: &Arc<AtomicBool>,
    key: &crossterm::event::KeyEvent,
) {
    let Some((handle, cfg)) = http_prewarm else {
        return;
    };
    ...
    drop(h.spawn(async move {
        nca_core::provider::validate::prewarm_default_provider(&cfg).await;
    }));
}
...
pub fn run_blocking(
    ...
    let http_prewarm_done = Arc::new(AtomicBool::new(false));
    ...
    loop {
        let version_changed = version_rx.has_changed().unwrap_or(false);
        {
            ...
            if !state_changed && !size_changed && !busy_tick_due {
                ...
                if let Some(sidebar) = sidebar_opt {
                    ...
                    let todos_header = if g.todos.is_empty() {
                        "todos".to_string()
                    } else {
                        format!(
                            "todos  {}/{}",
                            g.todos.completed_count(),
                            g.todos.len()
                        )
                    };
                    todo_lines.push(Line::from(Span::styled(
        ...
        if poll(Duration::from_millis(poll_ms))? {
            ...
            match ev {
                ...
                Event::Key(key) => {
                    schedule_http_prewarm_on_first_printable_key(
                        &http_prewarm,
                        &http_prewarm_done,
                        &key,
                    );
