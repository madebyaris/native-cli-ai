            ...
            for entry in timeline {
                let text = match entry {
                    ...
                    TimelineEntry::ApprovalResolved { approved, .. } => {
                        if *approved { "Approved".into() } else { "Denied".into() }
                    }
                    TimelineEntry::Cost {
                        input_tokens,
                        output_tokens,
                    ...
                    TimelineEntry::Checkpoint { phase, turn, .. } => format!("turn {turn}: {phase}"),
                    ...
                    TimelineEntry::ChildSpawned { child_session_id, task } => {
                        format!("child spawned: {child_session_id} - {}", truncate_chars(task, 60))
                    }
